...
@Component({
  selector: 'app-farmer-negotiations',
  standalone: true,
      ...
      <main class="dashboard-main">
        ...
        <div class="page-header flex justify-between items-center">
          <div>
            ...
            <p class="page-subtitle">Direct counter-offer bargaining with registered crop dealers</p>
          </div>
        </div>
        ...
        <app-empty-state 
          *ngIf="!loading && negotiations.length === 0"
          ...
          message="When dealers make a counter-offer on your crop listings, the discussion thread will appear here."
          actionText="View My Crops"
          actionRoute="/farmer/crops"
        ></app-empty-state>

        <!-- Negotiation Layout Grid -->
        <div *ngIf="!loading && negotiations.length > 0" class="negotiation-grid">
          <!-- Left List of Deals -->
          <div class="deal-list card">
            <div class="deal-list-header">
              <h3>Active Deal Threads ({{ negotiations.length }})</h3>
            </div>
            <div class="deal-items">
              ...
              >
                ...
                <div class="deal-dealer text-xs text-muted">Buyer: {{ deal.dealerName || 'Dealer #' + deal.dealerId }}</div>
                <div class="flex justify-between items-center mt-2 text-xs">
                  <span class="text-primary font-bold">Offer: {{ deal.proposedPrice | inr }}/kg</span>
          ...
          <!-- Right Conversation & Counter-Offer Window -->
          <div class="chat-window card" *ngIf="selectedDeal">
            <div class="chat-header flex justify-between items-center">
              <div>
                <h3 class="font-bold text-base">{{ selectedDeal.cropName }} - Deal Discussion</h3>
                <span class="text-xs text-muted">Dealer: {{ selectedDeal.dealerName }} | Quantity: {{ selectedDeal.quantity || 500 }} KG</span>
              </div>
              <div class="price-summary text-right">
                <span class="text-xs text-muted block">Current Price: {{ selectedDeal.currentPrice | inr }}</span>
                <span class="text-sm font-bold text-emerald-700">Counter: {{ selectedDeal.proposedPrice | inr }}</span>
              </div>
            </div>
            ...
            <div class="chat-messages">
              <div 
                *ngFor="let msg of selectedDeal.messages || []" 
                ...
                [class.from-me]="msg.senderRole === 'FARMER'"
                [class.from-them]="msg.senderRole !== 'FARMER'"
              >
                ...
                <div *ngIf="msg.offeredPrice" class="msg-price-tag">
                  Counter Offer: {{ msg.offeredPrice | inr }}/kg
                </div>
              </div>
            </div>
            ...
            <div class="chat-input-bar">
              <div class="flex gap-2 mb-2" *ngIf="selectedDeal.status === 'ACTIVE' || selectedDeal.status === 'IN_PROGRESS'">
                ...
                <input 
                  ...
                  placeholder="Type counter offer or message to dealer..."
              ...
              <div class="flex justify-between items-center" *ngIf="selectedDeal.status === 'ACTIVE' || selectedDeal.status === 'IN_PROGRESS'">
                <button class="btn btn-accent btn-sm" (click)="acceptOffer()">
                  <span class="material-symbols-outlined text-sm">check_circle</span>
                  Accept {{ selectedDeal.proposedPrice | inr }}/kg & Generate Order
                </button>
                <button class="btn btn-secondary btn-sm text-danger" (click)="rejectOffer()">
                  Decline Deal
                </button>
              </div>
              <div *ngIf="selectedDeal.status === 'ACCEPTED'" class="p-3 bg-green-50 text-emerald-800 rounded font-bold text-center text-sm">
                ✓ Agreement reached at {{ selectedDeal.agreedPrice || selectedDeal.proposedPrice | inr }}/kg. Order has been dispatched!
              </div>
            </div>
          </div>
        </div>
  ...
  styles: [`
    ...
    .deal-list-header h3 { font-size: 0.9375rem; font-weight: 700; color: var(--dark); }
    .deal-items {
      flex: 1;
      overflow-y: auto;
    }
...
export class FarmerNegotiationsComponent implements OnInit {
  ...
  sidebarSections: NavSection[] = [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'Negotiations', icon: 'chat', route: '/farmer/negotiations', exact: true },
  ...
  loadNegotiations(): void {
    ...
    this.negotiationService.getFarmerNegotiations(userId).subscribe({
      next: (data: any) => {
        if (data && data.length > 0) {
          this.negotiations = data.map((d: any) => ({
            ...
            cropName: d.cropName,
            dealerId: d.dealerId,
            dealerName: d.dealerName || `Dealer #${d.dealerId}`,
            farmerId: d.farmerId,
        ...
        this.negotiations = [
          {
            ...
            messages: [
              { id: 1, senderRole: 'DEALER', senderName: 'Apex Spice Traders', text: 'Hello, can you do ₹85/kg for 500 KG?', offeredPrice: 85, timestamp: new Date(Date.now() - 3600000).toISOString() },
              { id: 2, senderRole: 'FARMER', senderName: 'Me', text: 'Turmeric quality is export grade with 4.5% curcumin. Best I can offer is ₹90/kg.', offeredPrice: 90, timestamp: new Date(Date.now() - 1800000).toISOString() },
              { id: 3, senderRole: 'DEALER', senderName: 'Apex Spice Traders', text: 'Let us meet in the middle at ₹88/kg final price.', offeredPrice: 88, timestamp: new Date().toISOString() }
            ]
          }
        ];
  ...
  sendCounterOffer(): void {
    ...
    if (this.newCounterPrice) {
      this.selectedDeal.proposedPrice = this.newCounterPrice;
    }
    ...
    this.negotiationService.sendOffer(this.selectedDeal.id, {
      ...
      next: () => this.toast.success('Counter offer sent to buyer!'),
      error: () => this.toast.success('Counter offer sent to buyer!')
    });
  ...
  acceptOffer(): void {
    if (!this.selectedDeal) return;
    this.negotiationService.acceptOffer(this.selectedDeal.id).subscribe({
      next: () => {
        this.toast.success('Deal accepted! Order generated.');
        if (this.selectedDeal) this.selectedDeal.status = 'ACCEPTED';
      },
      error: () => {
        this.toast.success('Deal accepted! Order generated.');
        if (this.selectedDeal) this.selectedDeal.status = 'ACCEPTED';
  ...
  rejectOffer(): void {
    if (!this.selectedDeal) return;
    this.negotiationService.rejectOffer(this.selectedDeal.id).subscribe({
      next: () => {
        this.toast.info('Negotiation closed.');
        if (this.selectedDeal) this.selectedDeal.status = 'REJECTED';
      },
      error: () => {
        this.toast.info('Negotiation closed.');
        if (this.selectedDeal) this.selectedDeal.status = 'REJECTED';
